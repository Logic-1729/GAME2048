...
为此，你需要实现$\text{Game2048_Logic.cpp}$中的以下函数：
...
对于$\text{Extension}$部分，其最终得分取决于你的$\text{Average Score}$，对此，我们制定了以下公式用来换算你在本部分获得的分数:

$\begin{cases}
0,\text{score}<1000\\
\lceil \frac{\text{score}^2}{2\times 10^5}\rceil + 30,1000\leq \text{score} < 3000,35-75pts\\
\lceil12.27\log_2(\text{score}/1000) \rceil+55.55,3000\leq \text{score}<8000,75-93pts\\
\min(\lceil42.92\sqrt{\log_2(\text{score}/1000)} \rceil+18.16,110),\text{score}\geq 8000,93-110pts
\end{cases}$
